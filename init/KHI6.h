...
namespace PARAM{
	const double T_end = 0.35 * 8.0;
	const short int Ndim = 2;
	const double OUTPUT_TIME_INTERVAL = T_end / 200.0;
}
...
template <typename Ptcl> void SetupIC(vector<Ptcl>& ptcl, boundary_t<double>& boundary){
	...
	for(double x = 0 ; x < 1.0 ; x += dx){
		for(double y = 0 ; y < 1.0 ; y += dx){
			...
			vel.y = 0.025 * sin(12.0 * math::pi * x) * (exp(- (y - 0.25) * (y - 0.25) / (2.0 * sigma2)) + exp(- (y - 0.75) * (y - 0.75) / (2.0 * sigma2)));
